  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-7xl mx-auto px-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-6">
        {items.map((item) => (
          <div key={item.letter} className="text-center group flex flex-col items-center">
            {/* Circle */}
            <div className="relative w-20 h-20 sm:w-22 sm:h-22 lg:w-24 lg:h-24 mb-3 rounded-full overflow-hidden shadow-lg">
              ...
              <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                <span className="text-3xl lg:text-4xl font-bold text-white">{item.letter}</span>
              </div>
            </div>
            ...
            <p className="hidden sm:block text-xs text-gray-200 mt-1 leading-snug">{item.description}</p>
          </div>
        ))}
      </div>
    </section>
